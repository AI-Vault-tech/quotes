// add-final-batch-thousands-unique-quotes-to-database.js
const fs = require('fs');
...
// Read the final batch of thousands unique quotes
const finalBatchThousandsQuotes = JSON.parse(fs.readFileSync('final-batch-thousands-unique-philosophical-quotes.json', 'utf8'));
console.log(`Loaded ${finalBatchThousandsQuotes.length} final batch thousands unique quotes to add`);

// Add unique quotes to largeQuotes array
const updatedQuotes = [...largeQuotes, ...finalBatchThousandsQuotes];
...
console.log(`Successfully added ${finalBatchThousandsQuotes.length} final batch thousands unique quotes to largeQuotes.json`);
...
thematicContent = thematicContent.replace(/29,350\+/g, '36,350+');
...
footerContent = footerContent.replace(/29,350\+/g, '36,350+');
